...
export default function UploadBankReceiptDialog({
  ...
  const addFiles = (incoming: File[]) => {
    ...
    for (const f of incoming) {
      if (!f.type.startsWith("image/") && f.type !== "application/pdf") { errs.push(`فرمت فایل «${f.name}» مجاز نیست. فقط تصویر یا PDF مجاز است.`);continue; }
      if (f.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
        errs.push(`حجم «${f.name}» بیش از ${MAX_FILE_SIZE_MB}MB است (${formatBytes(f.size)}).`); continue;
      }
      acc.push(f);
    }
    if (acc.length > remaining) {
      errs.push(`حداکثر ${MAX_FILES} تصویر مجاز است. (می‌توانید ${remaining} عدد دیگر اضافه کنید)`);
  ...
  const handleUpload = async () => {
    if (!files.length) { setErrors((p) => [...p, "لطفاً حداقل یک تصویر انتخاب کنید."]); return; }
    try {
      ...
      setProgress(100);
      toast.success(res?.data?.message || "فیش بانکی با موفقیت ارسال شد.");
      handleClose();
    } catch (err: any) {
      console.error(err);
      setUploading(false);
...
<span style={{marginRight:20}}>آپلود فیش بانکی</span>
      ...
      <DialogContent dividers>
        <Stack spacing={3}>
<TextField
  placeholder="... توضیحی برای فیش بانکی وارد کنید"
  fullWidth
  multiline
  rows={3}
  dir="rtl"
...
/>
          ...
          >
            ...
            <Typography className="mt-2">تصاویر یا فایل فیش بانکی را بکشید و رها کنید یا کلیک کنید برای انتخاب</Typography>
          ...
          {!!errors.length && (
            <Box sx={{ bgcolor: "error.lighter", p: 2, borderRadius: 2, direction: "rtl" }}>
              {errors.map((er, i) => (
                <Typography key={i} variant="body2" color="error">• {er}</Typography>
              ))}
            </Box>
          )}
      ...
      <DialogActions sx={{ p: 2, direction: "rtl" }}>
        <Button onClick={handleClose} disabled={uploading}>بستن</Button>
        <Button
          variant="contained"
