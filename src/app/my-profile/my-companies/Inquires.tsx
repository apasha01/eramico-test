...
import { INQUIRY_COMPANY, INQUIRY_LIST } from "@/lib/urls";
...
const Inquires = ({ id,isMine }: { id: number,isMine:boolean }) => {
    ...
    const fetchData = async () => {
      try {
        const response = await axiosInstance.get<Inquires_res>(
          `${isMine ? INQUIRY_COMPANY :INQUIRY_LIST}?CompanyId=${id}&Size=${PAGE_SIZE}&Page=${pageNumber}`
        );
          ...
          >
            ...
            >
              <Typography className="fs-24 fw-500">
                متأسفانه استعلامی برای این شرکت پیدا نشد
              </Typography>
            </div>
          </div>
        )}
